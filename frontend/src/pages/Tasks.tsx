...
type TaskStatus = 'pending' | 'assigned' | 'running' | 'completed' | 'disputed' | 'cancelled' | 'failed'
...
export default function Tasks() {
  ...
  const stats = {
    ...
    failed: tasks.filter(t => t.status === 'cancelled' || t.status === 'disputed').length,
  }
          ...
          <div className="flex gap-2">
            <div className="relative">
              ...
              >
                ...
                <option value="disputed">{t('status.dispute')}</option>
              </select>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.codeUploadMethod === 'paste' && codeContent) {
        ...
        const ALLOWED_EXTENSIONS = ['.py', '.pyw', '.ipynb', '.txt', '.md', '.rst', '.json', '.jsonl', '.yaml', '.yml', '.toml', '.csv', '.tsv', '.xml', '.r', '.jl']
      ...
      if (formData.codeUploadMethod === 'paste') {
        ...
        // 对文本格式的代码文件也进行安全检�?
        const textExtensions = ['.py', '.pyw', '.txt', '.md', '.rst', '.json', '.jsonl', '.yaml', '.yml', '.toml', '.csv', '.tsv', '.xml', '.r', '.jl', '.ipynb']
        const fn = codeFile.name.toLowerCase()
        if (textExtensions.some(ext => fn.endsWith(ext))) {
      ...
      if (formData.enableEncryption) {
        ...
        if (result) {
          ...
          // 自动提交任务
          await encryptedTaskApi.submit(result.taskId)
          ...
          setSubmitResult({
            success: false,
            message: t('tasks.modal.createEncryptedTaskFailed')
          })
        }
      } else {
        // 使用普通任�?API
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {['H100', 'A100', 'RTX4090', 'RTX3090'].map((gpu) => (
                    <button
                      key={gpu}
